...
/// See also:
///
///  * [CustomListView], which is a commonly used [ScrollView] that displays a
...
abstract class ScrollView extends StatelessWidget {
  ...
  /// example [CustomListView] will use the number of widgets in the child list,
...
/// [CustomListView] or [CustomScrollView], the top item and all of the items below it
...
/// this information automatically. For example, [CustomListView] provides each child
...
class CustomScrollView extends ScrollView {
  ...
  /// to use a [SliverList] (maybe using a [CustomListView, which is a convenient
  ...
  ///
  /// The [CustomListView] and [GridView] widgets provide a convenient way to combine
...
/// See also:
///
///  * [CustomListView], which is a [BoxScrollView] that uses a linear layout model.
...
///
/// [CustomListView] is the most commonly used scrolling widget. It displays its
/// children one after another in the scroll direction. In the cross axis, the
/// children are required to fill the [CustomListView].
///
...
/// [controller] with its [ScrollController.initialScrollOffset] property set.
///
/// By default, [CustomListView] will automatically pad the list's scrollable
/// extremities to avoid partial obstructions indicated by [MediaQuery]'s
/// padding. To avoid this behavior, override with a zero [padding] property.
///
/// {@tool snippet}
/// This example uses the default constructor for [CustomListView] which takes an
/// explicit [List<Widget>] of children. This [CustomListView]'s children are made up
...
///
/// A [CustomListView] is basically a [CustomScrollView] with a single [SliverList] in
/// its [CustomScrollView.slivers] property.
///
/// If [CustomListView] is no longer sufficient, for example because the scroll view
...
/// [CustomListView] to using [CustomScrollView] directly.
///
/// The [key], [scrollDirection], [reverse], [controller], [primary], [physics],
/// and [shrinkWrap] properties on [CustomListView] map directly to the identically
...
///
/// The [childrenDelegate] property on [CustomListView] corresponds to the
/// [SliverList.delegate] (or [SliverFixedExtentList.delegate]) property. The
/// [CustomListView] constructor's `children` argument corresponds to the
...
/// like [CustomListView]s do. To reproduce the behavior, wrap the slivers in
/// [SliverSafeArea]s.
///
...
/// {@tool snippet}
///
/// Here are two brief snippets showing a [CustomListView] and its equivalent using
/// [CustomScrollView]:
///
/// ```dart
/// ListView(
...
/// [CustomListView] at build time with whatever widgets you need to show for the
/// empty list state:
///
/// {@tool snippet}
///
...
///
/// [CustomListView] has no built-in notion of a selected item or items. For a small
/// example of how a caller might wire up basic item selection, see
/// [ListTile.selected].
///
/// {@tool dartpad}
/// This example shows a custom implementation of [ListTile] selection in a [CustomListView] or [GridView].
...
class CustomListView extends BoxScrollView {
  ...
  /// [CustomListView] itself is created, it is more efficient to use the [CustomListView]
  ...
  /// it is more efficient to use the [CustomListView] constructor.
  ///
  ...
  /// explicitly. The [CustomListView] and [ListView.builder] constructors create a
...
///
/// To create a linear array of children, use a [CustomListView].
///
/// To control the initial scroll offset of the scroll view, provide a
/// [controller] with its [ScrollController.initialScrollOffset] property set.
///
...
///  * [SingleChildScrollView], which is a scrollable widget that has a single
///    child.
///  * [CustomListView], which is scrollable, linear list of widgets.
///  * [PageView], which is a scrolling list of child widgets that are each the
///    size of the viewport.
///  * [CustomScrollView], which is a scrollable widget that creates custom
///    scroll effects using slivers.
